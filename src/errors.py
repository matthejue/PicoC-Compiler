...
class Errors:
    class InvalidCharacterError(Exception):
        ...
        def __init__(self, found, found_pos):
            self.description = f"{CM().YELLOW}InvalidCharacterError{CM().RESET}: '{found}' is not a "\
                "permitted character"
            self.found = found
    ...
    class UnclosedCharacterError(Exception):
        ...
        def __init__(self, expected, found, found_pos):
            self.description = f"{CM().YELLOW}UnclosedCharacterError{CM().RESET}: Expected {expected},"\
                f" found {found}"
    ...
    class NoApplicableRuleError(Exception):
        ...
        def __init__(self, expected, found, found_pos):
            self.description = f"{CM().YELLOW}NoApplicableRuleError{CM().RESET}: Expected '{expected}'"\
                f", found '{found}'"
            self.expected = expected
            self.found = found
            self.found_pos = found_pos
    ...
    class MismatchedTokenError(Exception):
        ...
        def __init__(self, expected, found, found_pos):
            ...
            self.description = f"{CM().YELLOW}MismatchedTokenError{CM().RESET}: Expected '{expected}'"\
                f", found '{found}'"
            self.expected = expected
            self.found = found
            self.found_pos = found_pos
    ...
    class UnknownIdentifierError(Exception):
        """If Token shouldn't syntactically appear at this position"""
        def __init__(self, found, found_pos):
            self.description = f"{CM().YELLOW}UnknownIdentifierError{CM().RESET}: Identifier "\
    ...
    class RedefinitionError(Exception):
        def __init__(self, found, found_pos, first, first_pos):
            self.description = f"{CM().YELLOW}RedefinitionError{CM().RESET}: Redefinition of '{found}'"
            self.found = found
            self.found_pos = found_pos
            self.first = first
    ...
    class ConstReassignmentError(Exception):
        def __init__(self, found, found_pos, first, first_pos):
            self.description = f"{CM().YELLOW}ConstReassignmentError{CM().RESET}: Can't reassign a new "\
    ...
    class NoMainFunctionError(Exception):
        ...
        def __init__(self, fname):
            self.description = f"{CM().YELLOW}NoMainFunctionError{CM().RESET}: There's no main function"\
                f" in file '{fname}'"

    class MoreThanOneMainFunctionError(Exception):
        def __init__(self, first_pos, second_pos):
            self.description = f"{CM().YELLOW}MoreThanOneMainFunctionError{CM().RESET}: There're at "\
    ...
    class NotImplementedYetError(Exception):
        ...
        def __init__(self, feature_description):
            self.description = f"{CM().YELLOW}NotImplementedYet{CM().RESET}: The feature of using "\
