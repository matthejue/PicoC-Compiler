...
This document summarizes how the compiler processes PicoC source files, covering preprocessing, lexing, parsing, AST passes, and final RETI output.

The relevant implementation lives mainly in:

- [`src/preprocessor.py`](../src/preprocessor.py)
- [`src/option_handler.py`](../src/option_handler.py)
- [`src/passes/`](../src/passes/)
- [`src/ast_transformers.py`](../src/ast_transformers.py)
...
The preprocessing stage is implemented in [`src/preprocessor.py`](../src/preprocessor.py) and is called from [`src/option_handler.py`](../src/option_handler.py).
...
The entry point is [`OptionHandler.build_all`](../src/option_handler.py), which reads the CLI input files and compiles them.
...
`_compl(code)` in [`src/option_handler.py`](../src/option_handler.py) runs the parser frontend and then the lowering pipeline.
...
- The frontend is implemented in [`src/ast_transformers.py`](../src/ast_transformers.py), mainly by `TransformerPicoC`.
...
- Token output can still be produced even if AST construction later fails.
...
- `_dt_pass(...)` formats that parse tree for diagnostic output or file output.
...
- `transformer.build_ast(ts_tree, code)` in [`src/ast_transformers.py`](../src/ast_transformers.py) walks the Tree-sitter parse tree and builds the compiler's PicoC AST.
...
The pass pipeline is defined in [`src/passes/`](../src/passes/) and run from [`src/option_handler.py`](../src/option_handler.py).
...
Each source file is compiled separately up to `reti_blocks`. The combination of multiple compilation units is the linking stage in this project, and that linker logic lives in [`src/option_handler.py`](../src/option_handler.py).
...
Part of linking / multi-file handling is preparing one merged symbol table with final global addresses, but the actual replacement of symbolic names inside RETI instructions happens later in the final [`reti`](../src/passes/linking/reti_pass.py) pass.

Relevant place in the code:

- [`Passes._reti_instr()`](../src/passes/linking/reti_pass.py) resolves symbolic RETI operands with `self.symbol_table.resolve(...)`
...
- `preprocessor`: expand includes, handle `#pragma once`, and substitute simple macros
- `build_all`: compile each file separately up to RETI blocks
- `_insert_start_fun`: create a synthetic entry point and move global initializers there
- `_link`: merge per-file ASTs, block maps, and symbol tables
- `picoc_*` passes: normalize and lower PicoC into a stack-based intermediate form
- `reti_*` passes: turn the intermediate form into final RETI code
