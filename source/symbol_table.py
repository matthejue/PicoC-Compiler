    ...
    def load_json(cls, path: str | Path) -> "SymbolTable":
        ...
        for k, v in data.items():
            if k == "__parents__":
                ...
                st._table[k] = {
                    name: (
                        _restore_symbol_payload(dict(sym))
                        if isinstance(sym, dict)
                        else throw_error(
                            f"Invalid symbol table JSON: entry for symbol '{name}' in scope '{k}' is not an object"
                        )
                    )
...
def _json_default(ast):
    return _serialize_symbol_payload(ast)
...
_AST_NODE_TAG = "__ast_node__"


def _serialize_symbol_payload(value):
    if isinstance(value, pn.ASTNode):
        payload = {_AST_NODE_TAG: type(value).__name__}
        for attr_name, attr_value in vars(value).items():
            payload[attr_name] = _serialize_symbol_payload(attr_value)
        return payload
    if isinstance(value, dict):
        return {key: _serialize_symbol_payload(val) for key, val in value.items()}
    if isinstance(value, list):
        return [_serialize_symbol_payload(item) for item in value]
    return value


def _restore_symbol_payload(value):
    if isinstance(value, dict):
        node_name = value.get(_AST_NODE_TAG)
        if node_name is not None:
            ...
            for key, val in value.items():
                if key == _AST_NODE_TAG:
                    continue
                setattr(node, key, _restore_symbol_payload(val))
            return node
